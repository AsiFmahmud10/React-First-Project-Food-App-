...
const Recipe = () => {
  ...
  const url = `https://api.spoonacular.com/recipes/${foodId}/information?apiKey=9d1d7e5acfb344e7b0028ac0144e5c87&includeNutrition=false`;

  const [recipe, setRecipe] = useState(null);
  const [error, setError] = useState(null);

  useEffect(() => {
    if (foodId != null) getFoodFromApi(setRecipe, url, setError);
  }, []);
        ...
        <div className="recipe">
          ...
          <div className="recipe_ingredients">
            <div style={{ margin: "55px 0px" }}>
              <h2>{"ingredients".toUpperCase()} : </h2> <br />
              {item.map((item) => (
                <span
                  key={item.name}
